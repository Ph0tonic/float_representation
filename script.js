...
$(document).ready(()=>{
  ...
  $('#entry-whole').on('input',()=>{
    ...
    let result = "";
    result += '<td class="sign"><input class="input-binary" id="w_s" type="checkbox" '+(integer.sign?"checked":"")+'></td>';

    result += '<td class="main">';
    for(let i=0;i<integer.size;++i){
      result += '<input class="input-binary" id="w_v'+i+'" type="checkbox" '+(integer.value[i]?"checked":"")+'>\t';
    }
    result += '</td>';
  ...
  setTimeout(() => {
    ...
    $('#binary-whole').change('.input-float', ()=>{
      //Getting entry data
      let integer = new FloatingType(FloatingTypes[$("#floating-types").val()]);
      integer.sign = $('#f_s').prop('checked');

      for(let i=0;i<float.m;i++){
        integer.value[i] = $('#f_v'+i).prop('checked');
      }

      $('#entry-float').val(integer.toStr().toString());
    });

    $('#binary-float').change('.input-float', ()=>{
      //Getting entry data
      let float = new FloatingType(FloatingTypes[$("#floating-types").val()]);
      ...
      $('#entry-float').val(float.toStr().toString());
    });
