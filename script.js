...
$(document).ready(() => {
  ...
  $("#entry-whole").on("input", () => {
    ...
    for (let i = 0; i < integer.size; ++i) {
      result +=
        '<input class="input-binary" id="w_d' +
        i +
        '" type="checkbox" ' +
        (integer.value[i] ? "checked" : "") +
        ">\t";
    }
    result += "</td>";
  ...
  $("#entry-float").on("input", () => {
    float = new FloatingType(
      $("#entry-float").val(),
      FloatingTypes[$("#floating-types").val()]
    );

    //Formulaire de binary to decimal
    let result = "";
    result +=
      '<td class="sign"><input class="input-binary" id="f_s" type="checkbox" ' +
      (float.sign ? "checked" : "") +
      "></td>";
    ...
    for (let i = 0; i < float.e; ++i) {
      result +=
        '<input class="input-binary" id="f_e' +
        i +
        '" type="checkbox" ' +
        (float.exponent[i] ? "checked" : "") +
        ">\t";
    }
    result += "</td>";
    ...
    for (let i = 0; i < float.m; ++i) {
      result +=
        '<input class="input-binary" id="f_m' +
        i +
        '" type="checkbox" ' +
        (float.mantissa[i] ? "checked" : "") +
        ">\t";
    }
    result += "</td>";
