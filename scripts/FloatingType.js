...
class LogicOp {
  static minimise(a) {
    // Supprime les 0 non significatifs (à gauche)
    let n = 0;
...
class FloatingType {
  ...
  _initNaN() {
    ...
    let length = this.e;
    for (i = 0; i < length; ++i) {
      this.exponent.push(true);
    }

    this.mantissa.push(true);
  }
  ...
  isNaN() {
    ...
    return (
      this._exponentDecimal() + this.dOffset() == Math.pow(2, this.e) - 1 &&
      mantissaClone.length != 0
    );
  }
  ...
  isZero() {
    ...
    LogicOp.minimise(mantissaClone);
    return (
      this._exponentDecimal() + this.dOffset() == 0 && mantissaClone.length == 0
    );
  }
  ...
  _cleanMantissa() {
    for (let i = 0; i < this.m; ++i) {
      if (this.mantissa[i] != true) this.mantissa[i] = false;
    }
  }
  ...
  _init(value) {
    ...
    decimal = this._decimalToBinary(
      decimal,
      this.m + 1 + this.dOffset() + 1 - whole.length
    );
    ...
    if (exponent > -this.dOffset() && this.type !== FloatingTypes.EXTENDED) {
      ...
      exponent > -this.dOffset() &&
      this.type === FloatingTypes.EXTENDED
    ) {
      //Extended precision
    } else {
      //Subnormal numbers
  ...
  toString() {
    ...
    return result.times(this.sign ? -1 : 1).toString();
  }
  ...
  exponentDisplay() {
    // || this.type === FloatingTypes.EXTENDED
    return this._exponentDecimal() + (this.type !== FloatingTypes.EXTENDED && (this.isSubnormal() || this.isZero()) ? 1 : 0);
  }
}
